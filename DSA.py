...
from library import RandomGenerator as RandLib
...
def generate_p(p_Len, q):
    ...
    while True:
        ...
        p = q * temp + 1
        if number.isPrime(p) and RandLib.checkBitLength(p) == p_Len:
            if (p - 1) % q == 0:
                break
    print 'Found one!'
    return p
...
def generate_q(q_Len):
    ...
    while True:
        ...
        print 'q generated'
        if number.isPrime(q) and RandLib.checkBitLength(q) == q_Len:
            break
    return q
